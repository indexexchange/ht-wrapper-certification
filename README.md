...
* `test-partner-company-adapter` - Folder that contains the partner module (most of the development should be done in this folder)
    * `test-partner-company.js`                  - This is your partner module file, by default it contains a template divided into multiple sections which need to be completed.
...
1. <b>Git Setup</b>
    * Clone the repository, checkout the certification branch, and checkout appropriate submodule branch.
    ...
    git checkout test-partner-company-certification
    ...
    cd test-partner-company-adapter
...
2. <b>Complete the `test-partner-company.js` file in the test-partner-company-adapter folder.</b>
    * test-partner-company.js is where all of your adapter code will live.
...
3. <b>Committing changes to GitHub</b>
    * Please make sure that you're in the correct directory
    ...
    git push origin test-partner-company-certification
    ```
    ```sh
    # commit all the changes in the test-partner-company-adapter directory
    git commit -am "commit message"
...
5. <b>Submitting for Review</b>
    * Once the module has been verified, go to the [GitHub page](https://github.com/indexexchange/test-partner-company-adapter) of the partner adapter.
...
for (var i = 0; i < 10; i++) {
    ...
    if (w.headertag) {
        try {
            w.headertag.TestPartnerCompanyHtb.render(document, %%PATTERN:TARGETINGMAP%%, '%%WIDTH%%', '%%HEIGHT%%');
            break;
        } catch (e) {
            continue;
        }
...
3. Targeting is correctly applied and present on google tags `ads?` requests.
    * The partner specific key, `ix_tststt_cpm` for open/private market bids by price and `ix_tststt_dealid` for private market bids by deal id should be present.
...
4. Partner-Specific test ads are displaying.
    * In order to see your test ads displaying, you must win the auction in DFP.
        ...
        * Hence to see your ads displaying, you must return $2.00 bids or private market deals with dealId=`deal` that gets passed into dfp through your targeting key.
            * For bid by price example, `ix_tststt_cpm=300x250_200`
            * For bid by deal id example, `ix_tststt_dealid=300x250_deal`
