...
* `rhythm-one-adapter` - Folder that contains the partner module (most of the development should be done in this folder)
    * `rhythm-one.js`                  - This is your partner module file, by default it contains a template divided into multiple sections which need to be completed.
...
1. <b>Git Setup</b>
    * Clone the repository, checkout the certification branch, and checkout appropriate submodule branch.
    ...
    git checkout rhythm-one-certification
...
2. <b>Complete the `rhythm-one.js` file in the rhythm-one-adapter folder.</b>
    * rhythm-one.js is where all of your adapter code will live.
...
3. <b>Committing changes to GitHub</b>
    * Please make sure that you're in the correct directory
    ...
    git push origin rhythm-one-certification
    ```
    ```sh
    # commit all the changes in the rhythm-one-adapter directory
    git commit -am "commit message"
...
5. <b>Submitting for Review</b>
    * Once the module has been verified, go to the [GitHub page](https://github.com/indexexchange/rhythm-one-adapter) of the partner adapter.
...
3. Targeting is correctly applied and present on google tags `ads?` requests.
    * The partner specific key, `ix_rone_cpm` for open/private market bids by price and `ix_rone_dealid` for private market bids by deal id should be present.
...
4. Partner-Specific test ads are displaying.
    * In order to see your test ads displaying, you must win the auction in DFP.
        ...
        * Hence to see your ads displaying, you must return $2.00 bids or private market deals with dealId=`deal` that gets passed into dfp through your targeting key.
            * For bid by price example, `ix_rone_cpm=300x250_200`
            * For bid by deal id example, `ix_rone_dealid=300x250_deal`
